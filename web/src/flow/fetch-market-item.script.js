  ...
  return fcl
    .send([
      ...
      pub struct SaleItem {
        ...
        init(itemID: UInt64, typeID: UInt64, owner: Address, price: UFix64, tokenID: UInt8) {
          self.itemID = itemID
          self.typeID = typeID
          self.owner = owner
          self.price = price
      ...
      pub fun main(address: Address, id: UInt64): SaleItem? {
        if let collection = getAccount(address).getCapability<&KittyItemsMarket.Collection{KittyItemsMarket.CollectionPublic}>(KittyItemsMarket.CollectionPublicPath).borrow() {
          if let item = collection.borrowSaleItem(itemID: id) {
            return SaleItem(itemID: id, typeID: item.typeID, owner: address, price: item.price, tokenID: item.tokenID)
          }
        }
        return nil
      }
